// Godonia host script. Do not move to a class library. Package version: 1.1.0
#if TOOLS
using Godot;
...
/// <summary>
/// Godot dock host for a Godonia editor plugin page.
...
public partial class AvaloniaEditorHost : AvaloniaControl, IAvaloniaEditorHost {
	...
	private void TryBind() {
		if (string.IsNullOrEmpty(PluginId) && HasMeta("godonia_plugin_id"))
			PluginId = GetMeta("godonia_plugin_id").AsString();

		if (string.IsNullOrEmpty(PluginId))
			return;

		SetMeta("godonia_plugin_id", PluginId);
