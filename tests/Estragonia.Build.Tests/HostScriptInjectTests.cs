...
public sealed class HostScriptInjectTests {
	...
	public void EmptyGodotProject_InsertsHostScripts_AndCompilesProbe() {
		...
		Assert.Contains("ESTRAGONIA001", result.Output);
		Assert.True(File.Exists(Path.Combine(dir.Path, "AvaloniaControl.cs")));
		Assert.True(File.Exists(Path.Combine(dir.Path, "UiHost.cs")));
	}
	...
	public void ExistingFiles_AreNotOverwritten() {
		...
		var result = dir.Build();
		Assert.True(result.Success, result.Output);
		Assert.DoesNotContain("ESTRAGONIA001", result.Output);
	...
	public void ClassLibrary_DoesNotInsertHostScripts() {
		...
		dir.WriteFile("Lib.csproj", """
			<Project Sdk="Microsoft.NET.Sdk">
				...
				<ItemGroup>
					<PackageReference Include="Ouse.Estragonia" Version="1.1.0" />
				</ItemGroup>
			</Project>
			""");
	...
	[Fact]
	public void InjectDisabled_EmitsEstragonia002_AndDoesNotCopy() {
		using var dir = TempProject.Create();
		dir.WriteNugetConfig(_nupkg.NupkgsDir);
		...
		var result = dir.Build();
		Assert.True(result.Success, result.Output);
		Assert.Contains("ESTRAGONIA002", result.Output);
		Assert.False(File.Exists(Path.Combine(dir.Path, "AvaloniaControl.cs")));
		Assert.False(File.Exists(Path.Combine(dir.Path, "UiHost.cs")));
	}
...
public sealed class LocalNupkgFixture : IDisposable {
	...
	public LocalNupkgFixture() {
		NupkgsDir = Path.Combine(Path.GetTempPath(), "estragonia-nupkgs-" + Guid.NewGuid().ToString("N"));
		Directory.CreateDirectory(NupkgsDir);

		var pack = ProcessRunner.Run(
			"dotnet",
			$"pack \"{Path.Combine(Repo.Root, "src", "JLeb.Estragonia", "JLeb.Estragonia.csproj")}\" -c Release -o \"{NupkgsDir}\" --nologo",
			Repo.Root);
		if (pack.ExitCode != 0)
...
internal sealed class TempProject : IDisposable {
	...
	public static TempProject Create() {
		var path = System.IO.Path.Combine(System.IO.Path.GetTempPath(), "estragonia-inject-" + Guid.NewGuid().ToString("N"));
	...
	public void WriteNugetConfig(string nupkgsDir) {
		...
		WriteFile("nuget.config", $"""
			...
			<configuration>
				<packageSources>
					<clear />
					<add key="estragonia-local" value="{escaped}" />
	...
	public void WriteGodotCsproj(bool? inject, string? extraDir) {
		var injectProp = inject switch {
			true => "<EstragoniaInjectHostScripts>true</EstragoniaInjectHostScripts>",
			false => "<EstragoniaInjectHostScripts>false</EstragoniaInjectHostScripts>",
			null => ""
		};
		var dirProp = extraDir is null
			? ""
			: $"<EstragoniaHostScriptsDir>$(MSBuildProjectDirectory)\\{extraDir}</EstragoniaHostScriptsDir>";
